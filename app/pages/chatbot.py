...
from llama_index import PromptTemplate, StorageContext, load_index_from_storage, ServiceContext
from llama_index.llms import HuggingFaceLLM
import torch
from transformers import BitsAndBytesConfig
import configparser

def init_llm(model_name, context_window, max_new_tokens, temp, top_k, top_p, embed_model):
...
def load_vector_store_and_context(role):
    if role == 'leg':
...
def app():
    ...
    llm = init_llm(model_name, context_window, max_new_tokens, temp, top_k, top_p, embed_model)
    storage_context = load_vector_store_and_context(role)
